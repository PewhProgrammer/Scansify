...
HRESULT LoadBinarySTLMeshFile( LPOLESTR lpOleFileName,rt::BVH* scene, bool flipYZ)
{
	...
	unsigned char header[80] = { 0 };   // initialized all values to 0
	fread(&header, sizeof(unsigned char), ARRAYSIZE(header), meshFile);

	// Read number of triangles
	int numTriangles = 0;
	fread(&numTriangles, sizeof(int), 1, meshFile);


	// Sequentially read the normal, 3 vertices of the triangle and attribute, for each triangle
	for (unsigned int t = 0; t < numTriangles; ++t)
	{
		rt::Vector normal;

		// Read normal
		fread(&normal, sizeof(float), 3, meshFile);
		normal = rt::Vector(normal[0], -normal[1], -normal[2]); // flipYZ

		rt::Vector normals[3] = { normal,normal,normal };
		rt::Point points[3];
		...
		{
			rt::Point vertex;
			auto k = fread(&vertex, sizeof(float), 3, meshFile);

			points[v] = rt::Point(vertex[0],-vertex[1],-vertex[2]);
		}

		// Read attribute, unecessary
		unsigned short attribute = 0;
		fread(&attribute, sizeof(unsigned short), 1, meshFile);

		scene->add(new rt::SmoothTriangle(points, normals));
	}
...
HRESULT WriteBinarySTLMeshFile(INuiFusionColorMesh *mesh, LPOLESTR lpOleFileName, bool flipYZ)
{
    ...
    // Write the header line
    const unsigned char header[80] = "Binary .STL file generated by Scansify. github@pewhprogrammer";   // initialize all values to 0
...
HRESULT WriteAsciiObjMeshFile(INuiFusionColorMesh *mesh, LPOLESTR lpOleFileName, bool flipYZ)
{
    ...
    // Write the header line
    std::string header = "#\n# Binary .OBJ file generated by Scansify.github@pewhprogrammer\n#\n";
...
HRESULT WriteAsciiPlyMeshFile(INuiFusionColorMesh *mesh, LPOLESTR lpOleFileName, bool flipYZ, bool outputColor)
{
    ...
    // Write the header line
    std::string header = "ply\nformat ascii 1.0\ncomment file created by Scansify. github@pewhprogrammer\n";
    fwrite(header.c_str(), sizeof(char), header.length(), meshFile);
